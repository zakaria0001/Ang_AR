...
export class CocktailService {
  ...
  getIngredients():Observable<Array<string>>{ 
    ...
    .pipe(
      map(
        (result:{drinks:Array<{strIngredient1:string}>})=>{
    ...
    return this.http.get(url).pipe(
      map((result: any) => {
        ...
        if (!ingredients.length) {
          throw new Error(`Ingredient with name ${name} doesn't exist`);
        }
  
        // Assuming ingredients[0] contains the desired ingredient
